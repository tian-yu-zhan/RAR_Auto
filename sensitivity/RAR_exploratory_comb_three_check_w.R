...
# for (ind in 2){
  ...
  for (itt in 1:n.itt){
    ...
    comb.func.1 = function(s.z.vec.in, s.n.vec.in){
   
      vec.temp = s.z.vec.in[!is.na(s.z.vec.in)]
      w.temp = s.n.vec.in[!is.na(s.z.vec.in)]
      w.temp = rep(1/length(s.z.vec.in), sum(!is.na(s.z.vec.in)))*
        (sum(!is.na(s.z.vec.in))/length(s.z.vec.in))
      
      # com.temp = sum(sqrt(w.temp/sum(s.n.vec.in))*vec.temp)
      com.temp = sum(sqrt(w.temp/sum(w.temp))*vec.temp)
    ...
    if (n.adj>1){
      ...
      z.com.l = comb.func.1(l.z.vec, s.n.vec)
      z.com.m = comb.func.1(m.z.vec, s.n.vec)
      z.com.h = comb.func.1(h.z.vec, s.n.vec)
      z.com.lm = comb.func.1(lm.z.vec, s.n.vec)
      z.com.lh = comb.func.1(lh.z.vec, s.n.vec)
      z.com.mh = comb.func.1(mh.z.vec, s.n.vec)
...
if (select.ind==1) write.csv(output, "results_one_out_of_three_doses_w.csv")
if (select.ind==2) write.csv(output, "results_two_out_of_three_doses_w.csv")
