...
z.test.unknown.var = function(trt.vec.in, pbo.vec.in){
  test.ind = TRUE
  if ((length(trt.vec.in)<=1)|(sum(c(trt.vec.in, pbo.vec.in))==0)){
    test.ind = FALSE
  }
  if (!test.ind){
    ...
    t.test.fit = prop.test(c(sum(trt.vec.in), sum(pbo.vec.in)), c(length(trt.vec.in), length(pbo.vec.in)),
...
# for (ind in 2){
  ...
  for (itt in 1:n.itt){
    ...
    pbo.vec = rbinom(n.vec.initial[1], 1, rate.vec[1])
    l.vec = rbinom(n.vec.initial[2], 1, rate.vec[2])
    m.vec = rbinom(n.vec.initial[3], 1, rate.vec[3])
    h.vec = rbinom(n.vec.initial[4], 1, rate.vec[4])
    ...
    z.cohort.mat[1, ] = c(m,
                          z.test.unknown.var(l.vec, pbo.vec)$p,
                          z.test.unknown.var(m.vec, pbo.vec)$p,
                          z.test.unknown.var(h.vec, pbo.vec)$p,
                          z.test.unknown.var(c(l.vec,m.vec), pbo.vec)$p,
                          z.test.unknown.var(c(l.vec,h.vec), pbo.vec)$p,
                          z.test.unknown.var(c(m.vec,h.vec), pbo.vec)$p
    )
    ...
    for (i in 1:n.look){
        ...
        RAR.measure = c(z.test.unknown.var(l.vec, pbo.vec)$stats,
                        z.test.unknown.var(m.vec, pbo.vec)$stats,
                        z.test.unknown.var(h.vec, pbo.vec)$stats
                        )
          
        RAR.measure = RAR.measure + rnorm(length(RAR.measure), 0, 0.01)
        RAR.measure[is.na(RAR.measure)] = -10
      ...
      pbo.new.vec = rbinom(n.pbo.add, 1, rate.vec[1])
      l.new.vec = rbinom(n.l.add, 1, rate.vec[2])
      m.new.vec = rbinom(n.m.add, 1, rate.vec[3])
      h.new.vec = rbinom(n.h.add, 1, rate.vec[4])
    ...
    p.out.l.unweighted[itt] = z.test.unknown.var(l.vec, pbo.vec)$p
    p.out.m.unweighted[itt] = z.test.unknown.var(m.vec, pbo.vec)$p
    p.out.h.unweighted[itt] = z.test.unknown.var(h.vec, pbo.vec)$p

    p.out.lm.unweighted[itt] = z.test.unknown.var(c(l.vec, m.vec), pbo.vec)$p
    p.out.lh.unweighted[itt] = z.test.unknown.var(c(l.vec, h.vec), pbo.vec)$p
    p.out.mh.unweighted[itt] = z.test.unknown.var(c(m.vec, h.vec), pbo.vec)$p
    
    p.out.global[itt] = z.test.unknown.var(c(l.vec, m.vec, h.vec), pbo.vec)$p
    ...
    sample.pbo.equal = rbinom(n.pbo.equal, 1, rate.vec[1])
    sample.l.equal = rbinom(n.pbo.equal, 1, rate.vec[2])
    sample.m.equal = rbinom(n.pbo.equal, 1, rate.vec[3])
    sample.h.equal = rbinom(n.pbo.equal, 1, rate.vec[4])
    ...
    p.unadj.l = z.test.unknown.var(c(sample.l.equal), sample.pbo.equal)$p
    p.unadj.m = z.test.unknown.var(c(sample.m.equal), sample.pbo.equal)$p
    p.unadj.h = z.test.unknown.var(c(sample.h.equal), sample.pbo.equal)$p
  ...
  output$true_ES[ind] = paste(round(rate.vec*sqrt(n/4/2),2),collapse = ", ")
  output$true_mean[ind] = paste(round(rate.vec,3),collapse = ", ")
...
if (select.ind==1) write.csv(output, "results_bin_one_out_of_three_doses.csv")
if (select.ind==2) write.csv(output, "results_bin_two_out_of_three_doses.csv")
